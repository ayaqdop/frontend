import moveCore, { privateFunctions } from "../../src/actions/movers";
...
test("move ball", () => {
  ...
  privateFunctions.moveBall(testObjects, toPosition);
  expect(testObjects.ball.position).toEqual(toPosition);
  expect(testObjects.teams[0].moves).toEqual(4);
});
test("move player", () => {
  const testObjects = JSON.parse(JSON.stringify(gameObjects));
  const piece = { team: testObjects.teams[0].name, number: 9 };
  const toPosition = [11, 9];
  privateFunctions.movePlayer(testObjects, piece, toPosition);

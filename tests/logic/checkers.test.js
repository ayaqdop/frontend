...
const ballPosition = [12, 8];

test("remove position from an array of positions", () => {
  expect(privateFunctions.removeSelf([ballPosition, [12, 5]], [12, 5])).toEqual([[12, 8]]);
  expect(privateFunctions.removeSelf([ballPosition, [12, 5]], [12, 11])).toEqual([[12, 8], [12, 5]]);
});

test("can move vertically", () => {
  expect(privateFunctions.canMoveVertically(allPositions, ballPosition, [12, 5])).toBeTruthy();
  expect(privateFunctions.canMoveVertically(allPositions, [12, 10], [12, 11])).toBeTruthy();
});
